...
function normalizePhone(value: unknown): string {
  ...
  if (!/^[6-9]\d{9}$/.test(digits))
    throw new HttpsError('invalid-argument', 'Enter a valid Indian mobile number.');
  return `+91${digits}`;
}
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (count >= maxOrders) {
      throw new HttpsError(
        'resource-exhausted',
        'Too many orders were submitted from this device. Please wait a few minutes and try again.',
      );
    }
  ...
  async (request) => {
    ...
    const requested = input.items.map((raw) => {
      ...
      if (!Number.isInteger(qty) || qty < 1 || qty > 99)
        throw new HttpsError('invalid-argument', 'Quantity must be 1–99.');
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (!shopSnap.exists || shopSnap.data()?.active !== true)
        throw new HttpsError('failed-precondition', 'This shop is not accepting orders right now.');
      const productSnaps = await Promise.all(
        requested.map((i) => tx.get(shopRef.collection('products').doc(i.productId))),
      );
      const orderItems = requested.map((requestedItem, index) => {
        ...
        const p = snap.data()!;
        if (p.active !== true || p.availabilityStatus === 'out_of_stock')
          throw new HttpsError(
            'failed-precondition',
            `${String(p.name ?? 'A product')} is currently unavailable.`,
          );
        const unitPrice = typeof p.discountPrice === 'number' ? p.discountPrice : p.mrp;
        if (typeof unitPrice !== 'number' || unitPrice <= 0)
          throw new HttpsError('failed-precondition', 'A product has an invalid price.');
        return {
      ...
      if (existing.exists) {
        tx.update(customerRef, {
          name: customerName,
          address,
          village: village ?? FieldValue.delete(),
          landmark: landmark ?? FieldValue.delete(),
          lastOrderAt: now,
...
export const getCheckoutPrefill = onCall(
  { region: REGION, enforceAppCheck: ENFORCE_APP_CHECK },
  async (request) => {
    const auth = requireAuth(request.auth);
    ...
    const data = snap.data()!;
    const verifiedPhone =
      typeof auth.token.phone_number === 'string' ? auth.token.phone_number : undefined;
    ...
    return {
      ...
      village: data.village ? String(data.village) : undefined,
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (current === 'placed' && Array.isArray(request.data?.items)) {
        ...
        for (const raw of request.data.items as Array<{ productId?: unknown; qty?: unknown }>) {
          const id = nonEmpty(raw.productId, 'productId', 100);
        ...
        items = items.map((item) => ({
          ...item,
          qty: qtyMap.get(String(item.productId)) ?? Number(item.qty),
        }));
      ...
      const cancelReason =
        target === 'cancelled'
          ? nonEmpty(request.data?.cancelReason, 'Cancellation reason', 250)
          : undefined;
      tx.update(orderRef, {
        status: target,
        items,
        ...
        statusHistory: [
          ...(order.statusHistory ?? []),
          { status: target, at: now, note: cancelReason ?? null },
        ],
...
export const cancelOrder = onCall(
  ...
  async (request) => {
    ...
    const shopId = nonEmpty(request.data?.shopId, 'shopId', 100);
    const orderId = nonEmpty(request.data?.orderId, 'orderId', 100);
    const reason = nonEmpty(request.data?.reason, 'Cancellation reason', 250);
    const verifiedPhone =
      typeof auth.token.phone_number === 'string' ? auth.token.phone_number : undefined;
    if (!verifiedPhone)
      throw new HttpsError('permission-denied', 'Phone verification is required.');
    const ref = db.doc(`shops/${shopId}/orders/${orderId}`);
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      if (!snap.exists) throw new HttpsError('not-found', 'Order not found.');
      const data = snap.data()!;
      if (data.customerPhone !== verifiedPhone)
        throw new HttpsError('permission-denied', 'This order belongs to another phone number.');
      ...
      tx.update(ref, {
        ...
        statusHistory: [
          ...(data.statusHistory ?? []),
          { status: 'cancelled', at: now, note: reason },
        ],
        updatedAt: now,
        updatedBy: auth.uid,
      });
...
export const registerOwnerFcmToken = onCall(
  { region: REGION, enforceAppCheck: ENFORCE_APP_CHECK },
  async (request) => {
    const auth = requireAuth(request.auth);
    const shopId = nonEmpty(request.data?.shopId, 'shopId', 100);
    const token = nonEmpty(request.data?.token, 'token', 4096);
    await assertOwner(auth.uid, shopId);
    const id = Buffer.from(token).toString('base64url').slice(0, 120);
    await db
      .doc(`owners/${auth.uid}/devices/${id}`)
      .set({ token, shopId, updatedAt: Timestamp.now() }, { merge: true });
    return { ok: true };
  },
);

export const notifyOwnerOnNewOrder = onDocumentCreated(
  { region: REGION, document: 'shops/{shopId}/orders/{orderId}' },
  async (event) => {
    const shopId = event.params.shopId;
    const orderId = event.params.orderId;
    const shop = await db.doc(`shops/${shopId}`).get();
    const ownerUid = shop.data()?.ownerUid;
    if (!ownerUid) return;
    const devices = await db.collection(`owners/${ownerUid}/devices`).get();
    ...
    const response = await getMessaging().sendEachForMulticast({
      tokens,
      notification: {
        title: 'New AgriConnect order',
        body: `${String(order?.customerName ?? 'Farmer')} placed an order of ₹${Number(order?.subtotal ?? 0).toLocaleString('en-IN')}`,
      },
      webpush: { fcmOptions: { link: `/owner/orders/${orderId}` } },
    });

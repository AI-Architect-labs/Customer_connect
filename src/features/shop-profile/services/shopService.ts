...
export async function saveShopProfile(
  ...
  let shopPhotoURL = existing?.shopPhotoURL;
  let shopPhotoStoragePath = existing?.shopPhotoStoragePath;
  if (photo) {
  ...
  const data = {
    ...
    ownerUid,
    shopPhotoURL,
    shopPhotoStoragePath,
    createdBy: existing?.createdBy ?? ownerUid,
    updatedBy: ownerUid,
    deletedAt: existing?.deletedAt ?? null,
    deletedBy: existing?.deletedBy ?? null,
  };
  if (existing) await updateShop(shopId, data);
  else await createShop(shopId, data);
}
export { getShop };
